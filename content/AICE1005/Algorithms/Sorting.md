### Bubble Sort
*Bubble sort* is a stable, in-place sorting algorithm that is generally not used due to its poor performance, even compared with other algorithms that share its [big-O complexity](./Time%20Complexity#Sorting). Its only merits are its name and simplicity to implement.
##### Method
