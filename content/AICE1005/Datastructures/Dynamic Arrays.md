...
We can see plainly that these requirements are actually the same as the [*list* abstract data type](Datastructures/Abstract%20Data%20Types.md#Lists), so we will be attempting to implement one, ideally using arrays for speed. Due to that implementation detail, where we will allocate more space than we are using, we need to make sure that we distinguish between the remaining `capacity()` and the current `size()` of the list, increasing the `capacity()` when needed.
