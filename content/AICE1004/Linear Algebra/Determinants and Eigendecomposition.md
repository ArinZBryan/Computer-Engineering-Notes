...
Where $a_{i,j}$ is the entry at $i, j$ in matrix $\mathbf{A}$, and $m_{i,j}$ is the determinant of the submatrix gained by removing the row $i$ and column $j$ from $\mathbf{A}$, or otherwise the $i, j$-th entry in the [matrix of minors](./Inverse%20Matrices#A-Level%20Method) of $\mathbf{A}$.
