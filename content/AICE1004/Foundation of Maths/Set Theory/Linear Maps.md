...
If a matrix represents a linear map that is [bijective (injective and surjective)](Foundation%20of%20Maths/Set%20Theory/Functions.md#Types%20Of%20Function), then it is non-singular. 
...
To compute the [range space](Foundation%20of%20Maths/Set%20Theory/Isomorphisms.md#Range%20Space) and [null space](Foundation%20of%20Maths/Set%20Theory/Isomorphisms.md#Null%20Space) of a linear map from its associated matrix form is simple. Simply setup the equation $M\cdot\begin{pmatrix}x\\ y\\ z\end{pmatrix} = \begin{pmatrix}a\\ b\\ c\end{pmatrix}$, where $M$ is the matrix form of the linear map.
...
![Arrow diagram showing the changing of basis of a linear map|float-right](images/Arrow%20Diagram%20-%20Map%20Representation.png)Let us derive such a matrix. Assuming we have a mapping $h$, represented between basis $B$ and $D$ using matrix $H$. We want a matrix $\hat H$ that represents the same mapping, but from $\hat B$ to $\hat D$, as can be seen in the arrow diagram.
